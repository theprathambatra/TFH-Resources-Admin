...
export async function PATCH(request, { params }) {
  try {
    ...
    const { data: current, error: loadError } = await auth.supabase.from("batches").select("*").eq("id", id).maybeSingle();
    if (loadError) throw loadError;
    if (!current) return NextResponse.json({ error: "Batch not found." }, { status: 404 });

    const body = await request.json();
    const payload = cleanBatch({ ...current, ...body });
    const { data, error } = await auth.supabase.from("batches").update(payload).eq("id", id).select("*").single();
    if (error) throw error;

    return NextResponse.json({ ok: true, batch: data });
  } catch (error) {
    console.error("admin update batch", error);
    const message = error?.message || "Could not update batch.";
    ...
    return NextResponse.json({ error: known ? message : "Could not update batch." }, { status: known ? 400 : 500 });
  }
}
